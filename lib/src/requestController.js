...
async function computeHealthChecks (checks, exposeFailure) {
  ...
  checks.forEach(({ label, evaluation }, index) => {
    ...
    if (evaluation && !equal(evaluation, results[index].value)) {
      healthChecks[label] = exposeFailure ? { status: 'FAIL', reason: `Evaluation "${JSON.stringify(evaluation)}" failed` } : 'FAIL';
      status = 500;
      return;
    }
