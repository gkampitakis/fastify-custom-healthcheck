...
describe('Request Controller', () => {
  ...
  it('Should compute health check and fail cause of evaluation', async () => {
    const payload = {
      stats: {},
      exposeFailure: true,
      healthChecks: [{ label: 'test', fn: () => false, evaluation: true }, { label: 'test2', fn: () => true }]
    };
  ...
  it('Should compute health check and use complex evaluation', async () => {
    const payload = {
      stats: {},
      exposeFailure: true,
      healthChecks: [{ label: 'test', fn: () => ({ some: { nested: { value: 'mock' } } }), evaluation: { some: { nested: { value: 'mock' } } } }, { label: 'test2', fn: () => true }]
    };
